...
class CreatePalPhieunhapTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('pal_phieunhap', function (Blueprint $table) {
            $table->engine = 'InnoDB';
            ...
            $table->dateTime('pn_ngayXacNhan')->default(NULL)->comment('Thời điểm xác nhận # Thời điểm xác nhận thanh toán phiếu nhập kho');
            $table->unsignedSmallInteger('nv_thuKho')->default('1')->comment('Thủ kho # Mã nhân viên (thủ kho/giám đốc), 1-chưa phân công');
            $table->dateTime('pn_ngayNhapKho')->default(NULL)->comment('Ngày nhập kho # Ngày nhập kho');
            ...
            $table->unsignedTinyInteger('pn_trangThai')->default('2')->comment('Trạng thái # Trạng thái phiếu nhập sản phẩm: 1-khóa, 2-lập phiếu, 3-thanh toán, 4-nhập kho');
            $table->unsignedSmallInteger('ncc_ma')->comment('Nhà cung cấp # ncc_ma # ncc_ten # Mã nhà cung cấp');

            $table->foreign('nv_nguoiLapPhieu') //cột khóa ngoại là cột `nv_nguoiLapPhieu` trong table `phieunhap`
                ->references('nv_ma')->on('pal_nhanvien'); //cột sẽ tham chiếu đến là cột `nv_ma` trong table `nhanvien`
            $table->foreign('nv_keToan') //cột khóa ngoại là cột `nv_keToan` trong table `phieunhap`
                ->references('nv_ma')->on('pal_nhanvien'); //cột sẽ tham chiếu đến là cột `nv_ma` trong table `nhanvien`
            $table->foreign('nv_thuKho') //cột khóa ngoại là cột `nv_thuKho` trong table `phieunhap`
                ->references('nv_ma')->on('pal_nhanvien'); //cột sẽ tham chiếu đến là cột `nv_ma` trong table `nhanvien`
            $table->foreign('ncc_ma') //cột khóa ngoại là cột `ncc_ma` trong table `phieunhap`
                ->references('ncc_ma')->on('pal_nhacungcap'); //cột sẽ tham chiếu đến là cột `ncc_ma` trong table `nhacungcap`
        });
        DB::statement("ALTER TABLE `pal_phieunhap` comment 'Phiếu nhập # Phiếu nhập'");
    }
    ...
    {
        Schema::dropIfExists('pal_phieunhap');
    }
}
